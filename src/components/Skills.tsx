    ...
    <section id="skills" className="relative py-20 md:py-28 overflow-hidden">
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal className="mx-auto max-w-3xl text-center space-y-4" variant="fade">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold">A systems-led practice that stays hands-on</h2>
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-2">
          {skillBuckets.map((bucket, index) => {
            ...
            return (
              <ScrollReveal key={bucket.title} delay={index * 140}>
                <div className="group h-full rounded-3xl border border-border/80 bg-card/80 p-7 backdrop-blur-lg shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-primary/60 hover:shadow-glow">
        ...
        <ScrollReveal className="mt-16 text-center space-y-4" delay={200} variant="fade">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {learningFocus.map((topic, index) => (
              <ScrollReveal
                key={topic}
                delay={index * 60}
                className="inline-flex rounded-full border border-primary/40 bg-primary/10 px-4 py-2 text-xs font-semibold text-primary shadow-sm"
                variant="scale"
              >
                {topic}
              </ScrollReveal>
